#!/usr/bin/env python3
"""
테스트 스니펫: parsing_with_date.py 사용 예시
"""

import json
from datetime import datetime, timezone, timedelta
from RAG.parsing_with_date import parse_with_date, filter_out_by_criteria
...
def test_basic_filtering():
    ...
    # 특정 날짜 필터링 (2025-09-30 제외)
    filtered = parse_with_date(events, {"date": "2025-09-30"})
    print(f"2025-09-30이 아닌 이벤트 수: {len(filtered)}")
    
    # 금요일 필터링 (금요일 제외)
    filtered = parse_with_date(events, {"weekday": 4})  # 4 = 금요일
    print(f"금요일이 아닌 이벤트 수: {len(filtered)}")
    
    # 특정 시간 필터링 (21:00 제외)
    filtered = parse_with_date(events, {"hour": "21:00"})
    print(f"21:00이 아닌 이벤트 수: {len(filtered)}")
...
def test_advanced_filtering():
    ...
    # 복합 조건: 2025년 10월의 금요일 21시 이벤트 제외
    filtered = parse_with_date(events, {
        "date": "2025-10-31",  # 10월 31일
        "weekday": 4,          # 금요일
        "hour": "21:00"        # 21시
    })
    print(f"복합 조건에 맞지 않는 이벤트 수: {len(filtered)}")
...
def test_time_window_filtering():
    ...
    # 특정 기준 시간에서 ±12시간 범위가 아닌 이벤트
    reference_time = datetime(2025, 10, 15, 12, 0, tzinfo=KST)
    filtered = parse_with_date(events, {
        "time_window_hours": 12,
        "reference_time": reference_time
    })
    print(f"기준 시간({reference_time})에서 ±12시간 범위가 아닌 이벤트 수: {len(filtered)}")
    ...
    print(f"기준 시간에서 가장 가까운 5개가 아닌 이벤트 수: {len(filtered)}")
    
    # 결과 출력 (처음 3개만)
    print("제외된 이벤트들 (처음 3개):")
...
def test_sorting():
    ...
    # 시작 시간순 정렬
    filtered = parse_with_date(events, {"date": "2025-09-01"}, sort_by="start")
...
def main():
    """메인 테스트 함수"""
    print("parsing_with_date.py 테스트 시작\n")
